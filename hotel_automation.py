from Utils.floor_utils import (
    create_floors,
    floor_movement_sub_corridor,
    change_ac_state_sub_corridor,
)
...
class HotelAutomation:
    def __init__(self, no_of_floors, main_corridors_per_floor, sub_corridor_per_floor):
        self.floor_map = create_floors(
            no_of_floors, main_corridors_per_floor, sub_corridor_per_floor
        )
    ...
    def movement_change(
        ...
        floor_id,
        sub_corridor_id,
        movement=hotel_management_constants["SENSORINPUT"]["MOVEMENT"],
    ):
        """
        ...
        """
        self.floor_map = floor_movement_sub_corridor(
            self.floor_map, floor_id, sub_corridor_id
        )
        sub_corridor_keys = list(
            self.floor_map[floor_id].floor_corridor_map["sub_corridors"].keys()
        )
        expected_power_consumption = self.floor_map[
            floor_id
        ...
        if not expected_power_consumption:
            if movement == hotel_management_constants["SENSORINPUT"]["MOVEMENT"]:
                ...
                for key in range(0, len(sub_corridor_keys)):
                    ...
                    self.floor_map = change_ac_state_sub_corridor(
                        self.floor_map, floor_id, sub_corridor_keys[key]
                    )
                    self.floor_map[floor_id].non_movement_sub_corridors.append(
                        sub_corridor_keys[key]
                    )
        ...
        if movement == hotel_management_constants["SENSORINPUT"]["NO_MOVEMENT"]:
            ...
            for key in range(0, len(affected_sub_corridors)):
                self.floor_map = change_ac_state_sub_corridor(
                    self.floor_map, floor_id, affected_sub_corridors[key]
                )
        return self.floor_map
