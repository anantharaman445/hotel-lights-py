...
class Floor:
    def __init__(self, main_corridors_per_floor, sub_corridor_per_floor):
        ...
        self.maximum_allowed_units = (self.total_main_corridors * 15) + (
            self.total_sub_corridors * 10
        )
    ...
    def __create_main_corridors(self):
        ...
        for i in range(0, self.total_main_corridors):
            self.floor_corridor_map["main_corridors"][i + 1] = MainCorridor(
                corridor_type["MAINCORRIDOR"]
            )

    def __create_sub_corridors(self):
        self.floor_corridor_map["sub_corridors"] = {}
        for i in range(0, self.total_sub_corridors):
            self.floor_corridor_map["sub_corridors"][i + 1] = SubCorridor(
                corridor_type["SUBCORRIDOR"]
            )
    ...
    def get_floor_equipment_units(self):
        ...
        for corridor_id, main_corridor in main_corridors.items():
            units_consumptionn = (
                units_consumptionn + main_corridor.compute_corridor_units()
            )

        for corridor_id, sub_corridor in sub_corridors.items():
            units_consumptionn = (
                units_consumptionn + sub_corridor.compute_corridor_units()
            )
    ...
    def change_sub_corridor_light_state(self, sub_corridor_id):
        self.floor_corridor_map["sub_corridors"][
            sub_corridor_id
        ].change_sub_corridor_light_state()

    def change_sub_corridor_ac_state(self, sub_corridor_id):
        self.floor_corridor_map["sub_corridors"][
            sub_corridor_id
        ].change_sub_corridor_airconditioner_state()
